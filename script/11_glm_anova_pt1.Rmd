...
This second video will show you how to build a between participants ANOVA model in R using the `{afex}` package.
...
And visualise the data - note here the use of using the `fct_reorder()` function to reorder the levels of our factor based on the RT. This can be useful to make our viusalisations more easily interpretable.
```{r}
rm_data_tidied %>%
...
We can use a modification of the `ggplot()` code we've used above to generate our visualisation. Note, we are filtering our the missing data using the `filter()` function before we start our plot.  We also specify that we're wanting to plot a combination of our two factors in the `aes()` definition using `Context:Sentence`. There are further things we could modify to improve the look of this graph. Can you figure out ways in which the labelling of the two factors could be clearer?
...
The next challenge is for you to work on the following questions on your own.
